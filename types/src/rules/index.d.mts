...
export type Rule = {
    ...
    next: string;
    /**
     * where the rule applies, as a sentence for the catalog ("a repository with a database"); absent means always
     */
    when?: string | undefined;
    /**
     * true where the rule applies; a string is the reason it does not (the finding is n/a with it), false a bare n/a
     */
    applies?: ((ctx: RepoContext) => boolean | string) | undefined;
    /**
     * the stages the rule belongs to (design, build, run); absent means every stage
     */
    stages?: import("./stage.mjs").Stage[] | undefined;
    /**
     * the strongest enforcement any machine can reach for this rule, and why it stops there; a rule already at its ceiling leaves the promotion queue instead of sitting in it forever
     */
    ceiling?: {
        at: Enforcement;
        why: string;
