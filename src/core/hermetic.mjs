 ...
 *
 *   TEST_DATABASE_URL set      the suite runs with DATABASE_URL pointed at it
...
export function suiteDatabase(dirs, o = {}) {
  ...
  if (db.url || file)
    return {
      ok: false,
      reason: `DATABASE_URL ${db.url ? "is set in this shell" : `is declared in ${file}`}, a database this run did not create: point TEST_DATABASE_URL at a throwaway one (a database in the container the integration suite starts will do) to run it here`,
    };
  return { ok: true, env: {} };
}
