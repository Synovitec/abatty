...
export const probes = [
  {
    ...
    approximates:
      "stands in for the compiler's own reading: a text reading, strings and comments blanked, of a postfix `!` after a name, a call or an index and before a member, an index, a closing bracket, a separator, an assignment or the end of the line; one before an operator (`x! + 1`) is missed rather than a `!` in JSX text counted",
    emptyScanOk: true,
    scan: (c) => {
      ...
      for (const f of files) {
        ...
        for (const m of text.matchAll(NON_NULL))
          findings.push({
            path: f,
    ...
    controls: [
      {
        name: "an assertion on an index, a member, a call, a compound assignment and a line's end",
        files: {
          "src/a.ts":
      ...
      {
        name: "comparisons, negations, a string, a comment and JSX text are not assertions",
        files: {
          "src/a.tsx":
