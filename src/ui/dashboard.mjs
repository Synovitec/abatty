...
function render() {
  ...
  $("#main").innerHTML =
    '<section class="hero">' + dial(r.score) +
      ...
      '<div class="chips">' + chip("ok", present, "present") + chip("warn", partial, "partial") + chip("bad", missing, "missing") + (na ? chip("na", na, "not applicable") : "") + chip(r.scrub.lines ? "bad" : "ok", r.scrub.lines, r.scrub.lines ? "lines naming a tool" : "no trace of the tools") + '</div>' +
