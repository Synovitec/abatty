...
| ID | Rule | Level | Insured by | Phase | Applies | Standard | Why |
|---|---|---|---|---|---|---|---|
| CODE-ESLINT | A linter configured, for every language in the tree | must | hard | 1 | a repository with sources (any language pack); at build, run | CODE.4 | The linter is where most rules of the standard become an error a machine refuses; without a config there is nothing to hold them. Each language pack names its linter; the rule keeps the same words. |
...
| CODE-DEADCODE | Dead code is a gate: files, dependencies, exports and types at zero issues | must | hard | 12 | a repository with sources (any language pack); at build, run | CODE.6 | Dead code is read, maintained and reasoned about for nothing; an unused dependency is an attack surface. knip at zero makes both a refusal instead of a cleanup. |
...
| CODE-FORMAT | A formatter configured and the format checked, for every language in the tree | must | hard | 1 | a repository with sources (any language pack); at build, run | CODE.4 | Formatting argued in review is attention taken from the change; one formatter, checked, ends the argument. |
...
| ID | Rule | Level | Insured by | Phase | Applies | Standard | Why |
|---|---|---|---|---|---|---|---|
| TEST-UNIT | A unit test runner and tests, for every language in the tree | must | hard | 2 / 10 | a repository with sources (any language pack); at build, run | TEST.1 | A change without a test that goes red on the bug is a change nobody can verify; the runner is the first feedback loop an agent has. |
