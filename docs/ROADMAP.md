...
repositories. Items 1 to 4, 6, 11 and 17 above are part of this list and are not repeated.

| #   | Change                                                                                                                                                                                                                                                                                                                                        | Evidence                                                                                                                                                                                                                                                        | Tier      | Size       |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- | ---------- |
| 18  | **Nothing outside the repository**: the standard and the plan come bundled with the profile the repository names, versioned, never as `../ops-hub/...`. Completes item 4                                                                                                                                                                      | `adoption.json` → `standard` and `plan` are relative paths into a sibling checkout; a repository cloned alone cannot read them                                                                                                                                  | blocker   | half a day |
...
| 28  | **The license and the standard in the open**: Apache-2.0 with a DCO, and `ops-hub/engineering` (the standard, the plan, the enforcement map, the lessons) moved into this package or a public standard repository with its own version; the catalog then cites a document anyone can read                                                     | `UNLICENSED`; the rules' reasons are here but the plan and the map live beside invoicing and legal files in a repository that can never be public; every "any project" item above assumes a reader can read the standard                                        | blocker   | half a day |
| 29  | **The enforced share as the main number**: of the rules present, the part held by a machine (hard, ratchet) against review and prose, per repository and per reading, on the status screen and the dashboard; the night's objective becomes moving one rule up a level (prose → review → ratchet → hard), the enforcement map's own procedure | The level exists on every rule (44 hard, 7 ratchet, 5 review, 9 prose in the catalog) but is only reported per rule, never as a share of the repository's standard; no other tool measures how much of a written standard is actually enforced                  | structure | half a day |
| 30  | **Provenance as the default, the scrub as an option**: `scrub` off by default and opt-in as white-label hygiene, with the opposite option beside it (a disclosure trailer on the commits an unattended run makes); the guard refuses nothing about disclosure unless the repository opted in                                                  | A tool that audits an agent's runs cannot also erase the evidence of them: this repository's own commit-msg hook refused an attribution trailer on 2026-09-15; open-source projects increasingly require disclosure, and a reversed vocabulary reads as evasion | blocker   | half a day |
