...
- `env`: `ADOPTION_CONFIG=.claude/adoption.json`.
...
The guard denies a daytime push to the base branch only when `adoption.json` says the repo
...
**Proven, and how.** The hooks are proven by `self-test.mjs` (104 checks, mutation-tested
seven ways). The runner loop is proven with [`templates/harness/testing/stub-agent.mjs`](../../templates/harness/testing/stub-agent.mjs)
