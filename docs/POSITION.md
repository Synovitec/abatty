...
| ------------------- | ---------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- |
| Reliability         | Trust collapses on the first false alarm and does not return                 | `npm run gate` green on itself, 143 tests (142 pass, 1 skipped), 0 runtime dependencies, the first dogfood found 9 real defects. But `abatty gate`, the package's own command, is RED here: see 2.1 | Qualified         |
| Latency             | A slow gate is a bypassed gate                                               | status 241 ms, `abatty measure` 254 ms, but the pre-push gate this repository actually runs is 196 s, dominated by the unit suite                                                                   | Risk              |
